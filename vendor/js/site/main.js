jQuery(function ($) {
  ...
  $(window)
    .on("resize", function () {
      ...
      if ($hero_height < $hero_content_height) {
        $(".hero_section").css("height", $hero_content_height + 70 + "px");
      }
      $(".hero_section").css(
        "padding-top",
        $hero_height / 2 - $hero_content_height / 2 + "px"
      );
    })
    .trigger("resize");
  ...
  if ($(".sticky_player").length != 0 && isStickyPlayer != "false") {
    NavOffset = playerHeight + 10;
  }
  if ($(".sticky_player").length != 0 && $("#sticktop").length != 0 && isStickyNav != "false" && isStickyPlayer != "false") {
    NavOffset = naviheight + playerHeight + 10;
  }
  $("body").attr("data-offset", parseInt(NavOffset) + 10);

  $(".navbar-nav a[href^='#'],.ScrollTo,.btn-scroll").on("click", function (e) {
    e.preventDefault();
    $("html, body")
      .stop()
      .animate(
  ...
  $(".sliderGallery,.trigger_slider").on("click", function (e) {
    e.preventDefault();
    var $this = $(this),
  ...
  if ($(".vegas-slides").length) {
    ...
    $(".vegas-controls a").on("click", function (e) {
      e.preventDefault();
  ...
  $(".playlist_expander").on("click", function (e) {
    e.preventDefault();
    $(".play_list").slideToggle();
  });
  if ($("#player-instance").length != 0) {
  ...
  if ($(".playlist-files").length) {
    ...
    for (var i = 0; i < playlist_items_length; i++) {
      ...
      $list_content
        .eq(i + 1)
        .children(".track_thumb")
        .html(
          "<img src=" +
            $playlist_audio.eq(i).attr("data-thumb") +
            ' alt="track Thumb" />'
        );
      $list_content
        .eq(i + 1)
        .children(".track_title")
        .html($playlist_audio.eq(i).attr("data-title"));
      $list_content
        .eq(i + 1)
        .children(".track_genre")
        .html($playlist_audio.eq(i).attr("data-genre"));
      $list_content
        .eq(i + 1)
        .children(".track_composer")
        .html($playlist_audio.eq(i).attr("data-artist"));
      $list_content
        .eq(i + 1)
        .children(".track_length")
        .html($playlist_audio.eq(i).attr("data-length"));
      $list_content
        .eq(i + 1)
        .find(".btn_watch_video")
        .attr("href", $playlist_audio.eq(i).attr("data-video"));
      $list_content
        .eq(i + 1)
        .find(".itunesLink")
        .attr("href", $playlist_audio.eq(i).attr("data-itunes"));
    }
    ...
    $(".play_it").on("click", function (e) {
      e.preventDefault();
      werock.play($(this).parents("li").index() - 1);
    });
    $(".audio-title").html(werock.playlist[0].title);
    $("#player-instance").on($.jPlayer.event.play, function (event) {
    ...
    $(".jp-volume li a").on("click", function (e) {
      e.preventDefault();
      $(".jp-volume li").removeClass("active");
      $("#player-instance").jPlayer(
        "volume",
        (($(this).parent().index() + 1) / 10) * 2
      );
  ...
  if ($("#contactform").length != 0) {
    $("#contactform").on("submit", function (e) {
      ...
      if (name === "" || !IsEmail(email) || message === "") {
        $("#valid-issue")
          .html("Prosimy o podanie prawidłowych danych.")
          .slideDown();
      } else {
        $.ajax({
          type: "POST",
          url: "admin/contact_requests",
          data: $("#contactform").serialize(),
          dateType: "json",
          success: function () {
            $("#contactform").slideUp();
            toastr.success("Wysłano prośbę o kontakt", "Sukces");
          },
        });
      }
    });
  ...
  $(".triggerTrack").on("click", function (e) {
    e.preventDefault();
    $(".trackLoading").show();
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        $("html, body")
          .stop()
          .animate(
            { scrollTop: $("#tracksAjaxWrapper").offset().top - NavOffset },
            1000,
            "swing"
          );
      },
  ...
  $(".closeTrackAjax").on("click", function (e) {
    e.preventDefault();
    $("html, body")
      .stop()
      .animate(
  ...
  $(".closeNewsAjax").on("click", function (e) {
    e.preventDefault();
    $("html, body")
      .stop()
      .animate(
  ...
  $(".triggerNews").on("click", function (e) {
    e.preventDefault();
    $(".trackLoading").show();
    var $this = $(this),
      href = $this.attr("href"),
    ...
    $.ajax({
      ...
      success: function (data) {
        var targetPageContent = $("<div />")
          .html(data)
          .find(".pageContentArea #xv-news" + key);
        ...
        $("html, body")
          .stop()
          .animate(
            { scrollTop: $("#newsAjaxWrapper").offset().top - NavOffset },
            1000,
            "swing"
          );
      },
